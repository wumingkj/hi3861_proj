...
// 常量定义
#define SENSOR_UPDATE_INTERVAL 1000    // 增加到2秒读取一次，减少干扰
#define LED_UPDATE_INTERVAL   100     // 500ms
...
static void Main_Task(void)
{
    ...
    // 启动提示音 - 使用普中科技的蜂鸣器报警函数
    //beep_alarm(2, 50);  // 2次报警，每次100ms
    
    // 初始化时间戳
    g_last_sensor_update = osKernelGetTickCount();
    g_last_led_update = osKernelGetTickCount();
    ...
    while (1) {
        ...
        // 3. 蜂鸣器控制（500ms间隔）- 使用普中科技的蜂鸣器
        if (current_time - g_last_buzzer_check >= BUZZER_CHECK_INTERVAL) {
            if (g_counter % 80 == 0) {  // 调整为80，减少蜂鸣频率
                // 使用普中科技的蜂鸣器报警函数
                //beep_alarm(1, 100);  // 1次报警，持续100ms
            }
            g_last_buzzer_check = current_time;
        }
        
        // 4. LED显示更新（100ms间隔，但只有数据变化时才更新）
        if (current_time - g_last_led_update >= LED_UPDATE_INTERVAL) {
            ...
            /*if (need_update) {
                ...
                if (g_dht11_connected) {
                    ...
                    OLED_RequestShowString(0, 10, "Check GPIO7", 8);
                }
