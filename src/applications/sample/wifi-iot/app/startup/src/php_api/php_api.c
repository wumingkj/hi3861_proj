...
static const char *g_webpage_html =
    ...
    "                    <input type=\"text\" id=\"device_name\" name=\"device_name\" value=\"Device\" required>"
    "                </div>"
"                <div class=\"form-group\">"
    ...
    "                    <input type=\"text\" id=\"bind_line\" name=\"bind_line\" value=\"Line1\" required>"
    "                </div>"
    "            </div>"
    "            <div class=\"section\">"
    "                <h2>WiFi配置</h2>"
    ...
    "                    <input type=\"text\" id=\"wifi_ssid\" name=\"wifi_ssid\" placeholder=\"输入WiFi名称\" required>"
    "                </div>"
    "                <div class=\"form-group\">"
    "                    <label for=\"wifi_password\">WiFi密码:</label>"
    "                    <input type=\"password\" id=\"wifi_password\" name=\"wifi_password\" placeholder=\"输入WiFi密码\">"
    "                </div>"
    "                <div class=\"form-group\">"
    "                    <label for=\"wifi_auth_type\">加密方式:</label>"
    "                    <input type=\"text\" id=\"wifi_auth_type\" name=\"wifi_auth_type\" value=\"WPA2\" placeholder=\"WPA2/WPA/WEP/开放网络\">"
    "                </div>"
    "            </div>"
    ...
    "            })"
    "            .catch(error => {"
    "                document.getElementById('statusMessage').innerHTML = '<div class=\"status error\">提交失败: ' + error + '</div>';"
    "            });"
    "        });"
    "        "
    "        // 加载当前配置（从KV存储读取）"
...
php_api_result_t php_api_get_system_status(php_api_system_status_t *status)
{
    ...
    // 运行状态
    char running_status_str[16];
    if (kv_get_string("running_status", running_status_str, sizeof(running_status_str)) == KV_SUCCESS)
    {
        status->running_status = atoi(running_status_str);
    }

    // 获取本地IP和WiFi状态
    const char *local_ip = php_api_get_local_ip();
    if (local_ip != NULL)
    {
        strncpy(status->local_ip, local_ip, sizeof(status->local_ip) - 1);
    }

    status->wifi_connected = php_api_is_wifi_connected();

    log_i("PHP_API", "系统状态读取成功: 设备=%s, 服务器IP=%s",
          status->device_name, status->server_ip);
...
php_api_result_t php_api_save_system_status(const php_api_system_status_t *status)
{
    ...
    // 保存运行状态
    char status_str[16];
    snprintf(status_str, sizeof(status_str), "%d", status->running_status);
    if (kv_set_string("running_status", status_str) != KV_SUCCESS)
    {
        log_e("PHP_API", "保存运行状态失败");
        return PHP_API_ERROR_MEMORY_ALLOC;
    }

    log_i("PHP_API", "系统状态保存成功: 设备=%s, 服务器IP=%s",
          status->device_name, status->server_ip);
...
php_api_result_t php_api_send_handshake(void)
{
    ...
    int json_len = snprintf(json_buffer, 512,
        ...
        "\"bind_line\":\"%s\","
        "\"running_status\":%d}",
        system_status.device_name,
        system_status.local_ip,
        system_status.user_id,
        system_status.bind_line,
...
php_api_result_t php_api_send_sensor_data(const sensor_data_t *sensor_data)
{
    ...
    int json_len = snprintf(json_buffer, 1024,
        ...
        sensor_data->smoke,
        sensor_data->relay_status ? "true" : "false",
        sensor_data->buzzer_status ? "true" : "false");
...
php_api_result_t php_api_send_json_data(const char *json_data)
{
    ...
    timeout.tv_sec = PHP_API_SEND_TIMEOUT_MS / 1000;
    timeout.tv_usec = (PHP_API_SEND_TIMEOUT_MS % 1000) * 1000;
    ...
    log_i("PHP_API", "成功连接到服务器: %s:%d", system_status.server_ip, PHP_API_SERVER_PORT);
    ...
    int request_len = snprintf(http_request, http_request_size,
        ...
        "\r\n"
        "%s",
        system_status.server_ip, PHP_API_SERVER_PORT,
    ...
    log_i("PHP_API", "HTTP请求发送成功，发送字节数: %d", (int)bytes_sent);
    ...
    if (bytes_received > 0) {
        ...
        log_i("PHP_API", "服务器响应: %s", response_buffer);
    }
...
static php_api_result_t php_api_handle_post_request(int client_socket, const char *request)
{
    ...
    return php_api_send_success_response(client_socket, "所有配置保存成功！设备将重新启动以应用WiFi配置。");
}
...
char* php_api_get_local_ip(void)
{
    ...
    struct netif *netif = netif_find("sta0");
    
    if (netif != NULL && netif_is_up(netif))
    {
        // 修复指针类型问题
        ip4addr_ntoa_r((const ip4_addr_t *)&netif->ip_addr, ip_addr, sizeof(ip_addr));
        return ip_addr;
    }
